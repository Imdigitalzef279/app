...
class MeterDetailScreen extends StatefulWidget {
  ...
  @override
  State<MeterDetailScreen> createState() =>
      _MeterDetailScreenState();
}
class _MeterDetailScreenState extends State<MeterDetailScreen> {
  ...
  Widget _buildOverviewSection(
    ...
    final now = DateTime.now();
    final chartData =
        context.watch<MeterChartCubit>().state;
    ...
    final double maxValue = limitedData.isEmpty
        ...
        ? double.tryParse(e.paramP) ?? 0
        : double.tryParse(e.paramEpi) ?? 0)
        .reduce((a, b) => a > b ? a : b);
    return Container(
              ...
              Row(
                ...
                children: [
                  Text(
                    latest != null
                        ? (chartType == ChartType.power
                        ? "${(double.tryParse(latest.paramP) ?? 0).toStringAsFixed(2)}"
                        : "${(double.tryParse(latest.paramEpi) ?? 0).toStringAsFixed(0)}")
                        : "--",
              ...
              Row(
                children: [
                  Expanded(
                    child: _metricCard(
                      "Điện áp",
                      "${(double.tryParse(latest?.paramUa ?? '') ?? 0).toStringAsFixed(0) ?? '--'}",
                      "V",
                    ),
                  ),
                  ...
                  Expanded(
                    child: _metricCard(
                      "Dòng điện",
                      "${(double.tryParse(latest?.paramIa ?? '') ?? 0).toStringAsFixed(0) ?? '--'}",
                      "A",
                    ),
                  ),
                ],
              ...
              Row(
                children: [
                  Expanded(
                    child: _metricCard(
                      "Công suất",
                      "${(double.tryParse(latest?.paramP ?? '') ?? 0).toStringAsFixed(1) ?? '--'}",
                      "kW",
                    ),
                  ),
                  const SizedBox(width: 10),
          ...
          BarChartData(
            barTouchData: BarTouchData(
              ...
              touchTooltipData: BarTouchTooltipData(
                ...
                getTooltipItem: (group, groupIndex, rod, rodIndex) {
                  ...
                  final time = item.updateTime ?? DateTime.now();
              ...
              bottomTitles: AxisTitles(
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (value, meta) {
                    ...
                    final time = limitedData[index].updateTime ?? DateTime.now();
                    String label;
            ...
            barGroups: List.generate(limitedData.length, (i) {
              ...
              final y = chartType == ChartType.power
                  ? (double.tryParse(item.paramP) ?? 0)
                  : (double.tryParse(item.paramEpi) ?? 0);
              ...
              bottomTitles: AxisTitles(
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (value, meta) {
                    ...
                    final time = data[index].updateTime ?? DateTime.now();
