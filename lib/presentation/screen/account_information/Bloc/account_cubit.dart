...
class AccountCubit extends Cubit<AccountState> {
  ...
  Future<bool> deleteAccount() async {
    try {
      emit(state.copyWith(status: LoadStatus.loading,request: Result(status: LoadStatus.loading)));
      ...
      if (response.data == null || response.data.toString() != uniqueName) {
        ...
        AppToast.showToastSuccess(title: "Tên người dùng không khớp hoặc không tồn tại.");
        return false;
      }

      emit(state.copyWith(status: LoadStatus.success, request: Result(status: LoadStatus.success)));
      AppToast.showToastSuccess(title: "Xóa tài khoản thành công!!");
      removeToken();
      return true;
    } catch (e) {
      emit(state.copyWith(status: LoadStatus.failure, error: "Đã có lỗi xảy ra!!!",request: Result(status: LoadStatus.failure)));
